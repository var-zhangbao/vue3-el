...
function cleanCacheLRU(fileToKeep) {
  return __async(this, [], function* () {
    ...
    const entries = [];
    yield Promise.all(fs.readdirSync(dir).map((entry) => new Promise((resolve) => {
    ...
    yield Promise.all(entries.slice(5).map((entry) => new Promise((resolve) => {
